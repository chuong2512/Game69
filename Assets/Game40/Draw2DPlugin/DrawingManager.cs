...
public class DrawingManager : MonoBehaviour
{
    ...
    [Header("Line Settings")] public Color colorStart;

    public Color colorEnd;

    public PhysicsMaterial2D material;

    [Range(0f, 10f)] public float widthStart;

    [Range(0f, 10f)] public float widthEnd;

    [Range(0f, 5f)] public float verticesDistance;
    ...
    [Range(0f, 10f)] public float massScale;

    [Range(-10f, 10f)] public float gravityScale;
    ...
    {
        this.mousePointer.GetComponent<TargetJoint2D>().target =
            Camera.main.ScreenToWorldPoint(this.getTouchPosition());
    }
    ...
    private void initCanFirstDraw()
    {
        ...
        if (this.mouseRay.collider != null && !mouseRay.collider.gameObject.CompareTag("Player"))
        {
            Debug.Log("Can Draw");
            this.canDraw = false;
        }
    ...
    {
        pathPolygonCollider.gameObject.SetActive(false);
    }
    ...
    private void makeLinePhysics()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < this.newVerticies.Count - 1; i++)
            {
                ...
                {
                    this.colliderAngle = Mathf.Atan2(this.newVerticies[i].y - this.newVerticies[i + 1].y,
                        this.newVerticies[i].x - this.newVerticies[i + 1].x);
                    this.colliderAngle += 1.57079637f;
                }

                float num = Mathf.Cos(this.colliderAngle);
                float num2 = Mathf.Sin(this.colliderAngle);
                float num3 = Mathf.Lerp(this.widthStart, this.widthEnd,
                    (float) i / (float) (this.newVerticies.Count - 2));
                this.newVerticies2.Add(new Vector2(this.newVerticies[i].x + num3 / 2f * num,
                    this.newVerticies[i].y + num3 / 2f * num2));
                this.newVerticies2.Insert(0,
                    new Vector2(this.newVerticies[i].x - num3 / 2f * num, this.newVerticies[i].y - num3 / 2f * num2));
            }
        ...
        {
            this.massCenterClone =
                UnityEngine.Object.Instantiate<GameObject>(this.massCenter, base.transform.position,
                    Quaternion.identity);
    ...
    private void Update()
    {
        ...
        this.hit = Physics2D.Raycast(this.mousePointer.transform.position, Vector2.zero, float.PositiveInfinity,
            this.layerMask);
        this.mouseRay = Physics2D.Raycast(Camera.main.ScreenToWorldPoint(this.getTouchPosition()), Vector2.zero,
        ...
        if (this.fixGetMouseButton() && checkPlayer)
        {
            this.onTouchMove();
        }

        if (this.fixGetMouseButtonUp() && checkPlayer)
        {
            this.onTouchUp();
            Level.Instance.Drop();
    ...
    {
        this.clone =
            UnityEngine.Object.Instantiate<GameObject>(this.path, this.mousePointer.transform.position,
                Quaternion.identity);
        ...
        this.pathLineRenderer.SetPosition(0,
            this.mousePointer.transform.position - new Vector3(0f, 0f, Camera.main.transform.position.z));
        this.newVerticies.Add(this.mousePointer.transform.position - this.clone.transform.position);
    ...
    {
        if (Vector2.Distance(this.mousePointer.transform.position, this.newVerticies_[this.posCount]) >
            this.verticesDistance)
        {
            if (this.canPassWhenDraw(this.newVerticies[this.newVerticies.Count - 1],
                    this.mousePointer.transform.position))
            {
                this.setBlockLineVisiable(false);
                this.posCount++;
                this.pathLineRenderer.SetVertexCount(this.posCount + 1);
                this.pathLineRenderer.SetPosition(this.posCount,
                    this.mousePointer.transform.position - new Vector3(0f, 0f, Camera.main.transform.position.z));
                this.newVerticies_.Add(this.mousePointer.transform.position - this.clone.transform.position);
                this.newVerticies.Add(this.mousePointer.transform.position - this.clone.transform.position);
                if (this.colliderType == ColliderTypeChoices.Edge_Collider)
                ...
                else
                {
                    ...
                    for (int i = 0; i < this.newVerticies.Count - 1; i++)
                    {
                        ...
                        this.newVerticies2.Add(new Vector2(this.newVerticies[i].x + num3 / 2f * num,
                            this.newVerticies[i].y + num3 / 2f * num2));
                        this.newVerticies2.Insert(0,
                            new Vector2(this.newVerticies[i].x - num3 / 2f * num,
                                this.newVerticies[i].y - num3 / 2f * num2));
                    }
            ...
            {
                this.setBlockLine(this.newVerticies[this.newVerticies.Count - 1],
                    Camera.main.ScreenToWorldPoint(this.getTouchPosition()));
            }
        }
    }
    ...
    private void CalculatesPrevCenterOfMassAndMass(float width)
    {
        ...
        {
            float num2 = Vector2.Distance(this.newVerticies[num], this.newVerticies[num + 1]) *
                         (float) this.massCenterPrecision;
            int num3 = 0;
